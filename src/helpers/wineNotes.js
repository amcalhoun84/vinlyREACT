...
export function LegumeNote(props) {
  return (
    <p>Legumes are a vast variety of seeds and pods that are used across the world in a nearly infinite variety of dishes. They generally require lighter wines, but try a medium bodied or even rich red can work wonders. Try a glass of Cabernet Sauvignon the next time you order Channa Malsala.</p>
  );
};
...
export function VegNote(props) {
  return (
    <p>Most commonly seen as sides or hor d'ouerves, vegetables actually pair with a great variety of white wines of nearly every body type.</p>
  );
};
...
export function BakedGoodsNote(prop) {
  return (
    <p>Frequently related to desserts, baked goods can also be savory. It really depends on the style and whether the filling is savory or sweet. Meat pies will pair with their respective meats, while sweet dessert baked goods go best with sweet or dry dessert wines.</p>
  );
};
...
export function BreadNote(prop) {
  return (
    <p>Yes! Wine pairs well with breads... we just don't have much of a selection at the moment. The author of this app is LEGITIMATELY doing research and having a great time of it. :)</p>
  )
}
